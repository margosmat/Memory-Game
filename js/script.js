...
// cardBacks array assigments
// so it have 8 pair of icons
for (let j = 0; j <= 8; j += 8) {
	for (let i = 0; i < 8; i++) {
...
// Shuffle the icons
// and assign them to the card elements in HTML
...
// Event listener for card clicking
$('.card').on('click', function(e) {
	// Checking if clicked card is already flipped
	let parent = $(e.target).parent();
	if (parent.hasClass('success')) return;
	if (!parent.hasClass('flipped')) {
		// Assigning 'flipped' class if card isn't flipped already
		...
		if ($('.flipped').length === 2) {
			...
			// Calling moves counter
			countMoves();
		}
	}
	// Checking if all pairs have been successfully guessed
	setTimeout(checkIfEndGame, 600);
});

// Unflipping card in case
// of wrong guess
function unflip() {
...
// Logic for showing the end game screen
...
// Constructing the end game stats
...
// Checks how much moves player made
// and refreshes stars if specific condition is met
...
// Timer start logic
// gameplay time is displayed in specific format
function startTimer() {
